...
risk-objective matrix is the three-expert majority matrix. Domain loads are derived
...
consensus = pd.read_csv(ROOT / 'expert_validation' / 'consensus_majority_matrix.csv').set_index('risk')
...
out = pd.DataFrame({'risk': codes, 'base_gap': b, 'systemic_relative_score': r})
out['rank'] = [int(ranks[c]) for c in codes]
out.to_csv(ROOT / 'results' / 'risk_scores.csv', index=False)

# Sensitivity grid and top-4 persistence.
rows = []
for coupling in np.round(np.arange(0, 0.8001, 0.01), 2):
    rel, rk = calculate(A, b, float(coupling))
    for i, c in enumerate(codes):
        rows.append({'lambda': float(coupling), 'risk': c, 'relative_score': float(rel[i]), 'rank': int(rk[c])})
sens = pd.DataFrame(rows)
sens.to_csv(ROOT / 'results' / 'sensitivity.csv', index=False)
summary = []
for c in codes:
    d = sens[sens.risk == c]
    summary.append({'risk': c, 'min_rank': int(d['rank'].min()), 'max_rank': int(d['rank'].max()), 'top4_persistence': float((d['rank'] <= 4).mean())})
pd.DataFrame(summary).to_csv(ROOT / 'results' / 'sensitivity_summary.csv', index=False)
for c in codes:
    sens[sens.risk == c][['lambda', 'relative_score', 'rank']].to_csv(ROOT / 'results' / f'sensitivity_{c}.csv', index=False)

# Leave-one-objective-out ablation.
rows = []
for j, obj in enumerate(objectives):
    rel, rk = calculate(np.delete(A, j, axis=1), b, 0.35)
    for i, c in enumerate(codes):
        rows.append({'objective_removed': obj, 'risk': c, 'relative_score': float(rel[i]), 'rank': int(rk[c])})
abl = pd.DataFrame(rows)
abl.to_csv(ROOT / 'results' / 'objective_ablation.csv', index=False)
summary = []
for c in codes:
    d = abl[abl.risk == c]
    summary.append({'risk': c, 'min_rank': int(d['rank'].min()), 'max_rank': int(d['rank'].max()), 'top4_all_ablations': int((d['rank'] <= 4).all())})
